// app/technology/analysis/[id]/page.tsx

import React from "react";
import { client } from "@/lib/sanity";
import { PortableText, PortableTextComponents } from "@portabletext/react";
import CommentsSection from "@/components/CommentsSection";

// 1. STYLE MAP (Indigo Theme for Tech)
const ptComponents: PortableTextComponents = {
  block: {
    normal: ({ children }) => (
      <p className="text-text-body text-lg leading-relaxed mb-6">{children}</p>
    ),
    // H2 headers are INDIGO
    h2: ({ children }) => (
      <h2 className="text-3xl font-bold text-text-heading mt-12 mb-6 border-l-4 border-card-tech pl-4">
        {children}
      </h2>
    ),
    h3: ({ children }) => (
      <h3 className="text-2xl font-bold text-text-heading mt-8 mb-4">
        {children}
      </h3>
    ),
    blockquote: ({ children }) => (
      <div className="bg-surface-muted p-8 rounded-xl border-l-4 border-card-tech my-8 shadow-sm">
        <p className="text-xl italic font-medium text-text-heading">
          {children}
        </p>
      </div>
    ),
  },
  list: {
    bullet: ({ children }) => (
      <ul className="list-disc pl-6 space-y-3 mb-6 text-text-body text-lg">
        {children}
      </ul>
    ),
    number: ({ children }) => (
      <ol className="list-decimal pl-6 space-y-3 mb-6 text-text-body text-lg">
        {children}
      </ol>
    ),
  },
  marks: {
    strong: ({ children }) => (
      <strong className="font-bold text-text-heading">{children}</strong>
    ),
  },
};

async function getArticle(slug: string) {
  const query = `*[_type == "policyAnalysis" && slug.current == $slug][0] {
    title, summary, publishedAt, body, pillar
  }`;
  return client.fetch(query, { slug }, { next: { revalidate: 0 } });
}

export default async function TechArticlePage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const article = await getArticle(id);

  if (!article) return <div>Article not found</div>;

  return (
    <div className="py-16 px-4 md:px-8 max-w-7xl mx-auto">
      <div className="max-w-4xl mx-auto text-center mb-16 border-b border-ui-border pb-12">
        <div className="flex justify-center gap-3 mb-6">
          <span className="px-3 py-1 bg-indigo-100 text-indigo-800 text-xs font-bold uppercase tracking-wider rounded-full">
            {article.pillar}
          </span>
          <span className="px-3 py-1 bg-surface-muted text-text-body text-xs font-bold uppercase tracking-wider rounded-full border border-ui-border">
            {new Date(article.publishedAt).toLocaleDateString()}
          </span>
        </div>
        <h1 className="text-4xl md:text-6xl font-extrabold text-text-heading mb-6 leading-tight">
          {article.title}
        </h1>
        <p className="text-xl text-text-body leading-relaxed mb-8 max-w-3xl mx-auto">
          {article.summary}
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        <div className="lg:col-span-8 lg:col-start-3">
          <div className="prose prose-lg max-w-none">
            <PortableText value={article.body} components={ptComponents} />
          </div>
          <hr className="my-12 border-ui-border" />
          <CommentsSection />
        </div>
      </div>
    </div>
  );
}
