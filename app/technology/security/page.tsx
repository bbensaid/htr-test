...
async function getSecurityArticles() {
  const query = `*[_type == "policyAnalysis" && pillar == "Technology" && category == "security"] | order(publishedAt desc) {
    _id,
    title,
    summary,
    publishedAt,
...
export default async function SecurityPage() {
  const articles = await getSecurityArticles();
    ...
    <div className="py-16 px-4 md:px-8 max-w-7xl mx-auto">
      <div className="mb-16 border-b border-ui-border pb-8">
        ...
        <p className="text-xl text-text-body leading-relaxed max-w-4xl">
          Deep dives into cybersecurity threats, ransomware resilience, and
          HIPAA compliance in the cloud age.
        </p>
      </div>
        ...
        {articles.length === 0 && (
          <div className="p-10 text-center bg-surface-muted rounded-xl border border-ui-border">
            ...
            <p className="text-sm text-text-body/60">
              Run the script with category: "security" to populate this list.
            </p>
          </div>
        )}
